...
import { FamilyMember, FinancialGoal, Investment, GoalInvitation } from '../types';
...
interface Store {
  ...
  inviteToGoal: (invitation: Omit<GoalInvitation, 'id' | 'createdAt' | 'status'>) => void;
...
export const useStore = create<Store>((set) => ({
  ...
  acceptInvitation: (invitationId) =>
    set((state) => {
      const invitation = state.goalInvitations.find(inv => inv.id === invitationId);
      ...
      return {
        ...
        goals: state.goals.map(goal =>
          goal.id === invitation.goalId
            ? { ...goal, contributors: [...goal.contributors, invitation.inviteeId] }
            : goal
        ),
      };
    }),
